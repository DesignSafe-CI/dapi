...
def plot_profile(data_dir=".", ndof=2, nstraincomp=3, nstresscomp=3):
    """
    Plot maximum displacement, PGA and maximum shear strain and maximum cyclic stress ratio
    """
    nodes = np.loadtxt(os.path.join(data_dir, "nodesInfo.dat"))
    disp = np.loadtxt(os.path.join(data_dir, "displacement.out"))
    acc = np.loadtxt(os.path.join(data_dir, "acceleration.out"))
    strain = np.loadtxt(os.path.join(data_dir, "strain.out"))
    stress = np.loadtxt(os.path.join(data_dir, "stress.out"))
